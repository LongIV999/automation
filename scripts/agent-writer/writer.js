...
async function main() {
    ...
    const systemPrompt = `
You are an expert Content Creator for ${brand === 'thachvuland' ? 'Thach Vu Land' : 'Long Best AI'}.
...

{
  "title": "String",
  "topic": "String",
  "brand": "${brand === 'thachvuland' ? 'Thach Vu Land' : 'Long Best AI'}",
  "slides": [
    {
...
3. Slide 7 must be type 'cta'.
   - Content: Quảng cáo giải pháp tạo Video & Ảnh AI chuyên nghiệp với Veo 3 và Nano Banana Pro.
   - Call to Action (CTA): Nhắn tin (ib) ngay để nhận bản dùng thử trước khi mua.
4. Content should be in Vietnamese.
...
`;
    ...
    try {
        ...
        const filename = `${brand}-${safeTopic}.json`;
